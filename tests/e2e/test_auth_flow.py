...
class TestAdminRequiredPaths:
    def test_non_admin_session_blocked_on_benchmark(self):
        ...
        with patch.multiple(
            ...
            client = TestClient(app, raise_server_exceptions=False)
            resp = client.get("/benchmark/results", cookies={"dewie_session": token})
        assert resp.status_code == 403

    def test_admin_session_passes_benchmark(self):
        token = _make_session_cookie(is_admin=True)
        with patch.multiple(
            "dewie.config.settings",
            auth_enabled=True,
