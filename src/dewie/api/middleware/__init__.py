...
def _scope_rule_denial(path: str, scopes: list[str]) -> Response | None:
    """Longest-prefix scope check shared by session and API-key auth paths.

    Returns a 403 Response when the path's route family requires a scope the
    caller doesn't hold, else None. Route handlers should still enforce any
    stricter rules they require.
    """
    ...
    if matched_scope is not None and not _has_scope(scopes, matched_scope):
        return Response(
...
async def _api_key_middleware(request: Request, call_next: Callable) -> Response:
    ...
    if getattr(settings, "local_auth_enabled", False) is True:
        ...
        request.state.actor_id = request.state.user_id
        return await call_next(request)
    ...
    if session_token:
        ...
        if payload:
            ...
            request.state.actor_id = request.state.user_id or payload.get("sub")
            # Honor the token's activation_status — pending/rejected users must
            # not reach protected APIs (previously hardcoded to "approved",
            # leaving enforcement to client-side JS only).
            activation = payload.get("activation_status", "approved")
            request.state.activation_status = activation
            if activation not in ("approved", "active"):
                return Response(
                    content='{"detail": "Account pending approval"}',
                    status_code=403,
                    media_type="application/json",
                )
            scope_denied = _scope_rule_denial(path, request.state.key_scopes)
    ...
    if not raw_key:
        # X-API-Key always wins when present; Bearer is only a fallback —
        # needed for MCP clients (stdio adapter + remote Streamable HTTP
        # transport), which send Authorization: Bearer rather than X-API-Key.
        auth_header = request.headers.get("Authorization", "").strip()
        if auth_header.lower().startswith("bearer "):
            raw_key = auth_header[7:].strip()
    if path in ("/admin/keys", "/api/admin/keys") and request.method == "POST" and not raw_key:
        # Bootstrap path: allow creating the very first API key from the admin UI
        # when no key exists yet (common fresh-install UX).
        ...
        if int(count or 0) == 0:
            # Fresh-install bootstrap: the route handler also enforces
            # is_admin, so grant it here rather than 403 the first key.
    ...
    request.state.actor_id = key_record.get("user_id") or str(key_record["id"])
